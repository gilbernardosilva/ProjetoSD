...
java -cp %CLASSPATH% -Djava.security.policy=%CLIENT_SECURITY_POLICY% -Djava.rmi.server.codebase=%SERVER_CODEBASE% -D%JAVAPACKAGEROLE%.codebase=%CLIENT_CODEBASE% -D%JAVAPACKAGE%.servicename=%SERVICE_NAME_ON_REGISTRY% edu.ufp.inf.sd.rmi.project.client.ProjectClient %REGISTRY_HOST% %REGISTRY_PORT% %SERVICE_NAME_ON_REGISTRY%
