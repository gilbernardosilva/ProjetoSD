...
public class Base {
    ...
    public enum Armor {FOOT, VEHICLE, TANK, AIR, SHIP, ALL, NONE}
    ...
    public boolean PathCheck(int destx, int desty) {
        ...
        if (destx >= Game.map.width || desty >= Game.map.height) {
            return false;
        }
        ...
        switch (legs) {//This is used to find out if the unit can move to said tile or not.
            case INF:
                if (Game.map.map[desty][destx].walk) {
                    return true;
                }
                break;
            case TIRE:
                if (Game.map.map[desty][destx].drive) {
                    return true;
                }
                break;
            case TANK:
                ...
                break;
            case SHIP:
                if (Game.map.map[desty][destx].swim) {
                    return true;
                }
                break;
            case FLY:
                if (Game.map.map[desty][destx].fly) {
                    return true;
                }
                break;
            default:
    ...
    public boolean inrange(int xx, int yy) {
        ...
        yy = (yy > y) ? yy - y : y - yy;
        if (xx + yy > MaxAtkRange) {
            return false;
        }
        if (xx + yy < MinAtkRange) {
            return false;
        }
        return true;
    }
